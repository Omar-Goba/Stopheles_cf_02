...
def train_model(model: keras.Model, tr_X: np.ndarray, tr_y: np.ndarray, epochs: int = 150, batch_size: int = 64) -> keras.Model:
    """
        trains the model
        args:
            model: keras model
...
def main():
    ...
    model.summary()
    train_model(model, tr_X, tr_y, epochs=150, batch_size= 64)
